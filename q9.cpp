...
public:
    ...
    bool addPacket(int source, int destination, int timestamp) {
        string k = makeKey(source, destination, timestamp);
        if (packetSet.count(k)) return false;
        if (packets.size() == memoryLimit) {
            auto &[src, dst, ts] = packets.front();
            packetSet.erase(makeKey(src, dst, ts));
            auto &vec = destToTimestamps[dst];
            vec.erase(lower_bound(vec.begin(), vec.end(), ts));
            packets.pop_front();
        }
        packets.emplace_back(source, destination, timestamp);
        packetSet.insert(k);
        auto &vec = destToTimestamps[destination];
        vec.insert(upper_bound(vec.begin(), vec.end(), timestamp), timestamp);
        return true;
    }
    ...
    int getCount(int destination, int startTime, int endTime) {
        auto it = destToTimestamps.find(destination);
        if (it == destToTimestamps.end()) return 0;
